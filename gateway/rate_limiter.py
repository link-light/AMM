...
class RateLimiter:
    ...
    async def acquire(
        ...
        await redis_client.incr(rpm_key)
        await redis_client.expire(rpm_key, 120)  # 2 minute TTL
        await redis_client.incrby(tpm_key, expected_tokens)
        await redis_client.expire(tpm_key, 120)
    ...
    async def record_cost(self, model_tier: str, cost: float):
        ...
        await redis_client.incrbyfloat(cost_key, cost)
        await redis_client.expire(cost_key, 172800)  # 2 day TTL
